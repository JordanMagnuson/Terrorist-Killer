    ...
    if (i!=0)
    {
        if (light_on[i]==true)
        {
            if (lightpref_view<0)
                ...
                draw_sprite_ext(light_sprite[i],light_frame[i],light_x[i]-__view_get( e__VW.XView, lightpref_view ),light_y[i]-__view_get( e__VW.YView, lightpref_view ),light_xs[i],light_ys[i],light_rot[i],light_color[i],1);
        }
    }
    i+=1;
}
...
if (lightpref_view<0)
    ...
    draw_surface_ext(lightpref_surface,__view_get( e__VW.XView, lightpref_view ),__view_get( e__VW.YView, lightpref_view ),1,1,0,c_white,1);
draw_set_blend_mode(0);
...
{
    if (sw!=__view_get( e__VW.WView, lightpref_view )||sh!=__view_get( e__VW.HView, lightpref_view ))
    {
        surface_free(lightpref_surface);
        lightpref_surface=surface_create(__view_get( e__VW.WView, lightpref_view ),__view_get( e__VW.YView, lightpref_view ));
    }
}
