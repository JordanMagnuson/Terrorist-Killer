function scr_step_typ(argument0) {
	if (room_speed > 30) odd_frame *= -1;	// Need to slow things down if 60 fps.
	if (odd_frame == 1) {
		...
		}// Check if user presses the defined key for skippping. If yes then set skip to true
		...
		if skip = false // Skip is not true then continue script as normal
		{
			if sst < string_length(ist)+1 // If text is not fully shown then
			{
				...
				if (!audio_is_playing(global.audio_typing)) 
